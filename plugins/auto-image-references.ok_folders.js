        ...
        if (fs.existsSync(staticFolder)) {
          ...
          if (usedImageReferences.length > 0) {
            ...
            if (missingReferences.length > 0) {
              ...
              const endsWithBlankLine = updatedContent.endsWith('\n\n');
              const endsWithReference = updatedContent.match(/\[.*\]: .*$/gm)?.length > 0;

              // If it ends with a blank line or a reference, we don't add another one
              if (!endsWithBlankLine && !endsWithReference) {
