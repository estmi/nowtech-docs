...
--			IF @Moneda=0 
			...
			if exists(select 1 from Albaranes_Cli_Cab where IdCliente=@IdCliente and FechaAlb=@FechaAlb and IdEstado=0)
				and exists(select 1 from Conf_Pedidos_Cli where IdPedido=@Ped and coalesce(P_AlbaranSeparado,0)=0) begin

				select top 1 @IdAlbaranI=IdAlbaran from Albaranes_Cli_Cab where IdCliente=@IdCliente and FechaAlb=@FechaAlb and IdEstado=0 
